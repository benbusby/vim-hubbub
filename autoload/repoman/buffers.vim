...
function! SetHeader(show_page_num, state) abort
    let l:line_idx = 1
    for line in readfile(g:repoman_dir . '/assets/header.txt')
        let l:page_id = a:show_page_num ? ' (page ' . a:state.page . ')' : ''
        if l:line_idx == 1
...
function! OpenBuffer(buf_name, show_page_num, state) abort
    ...
    return a:show_page_num >= 0 ? 
        \SetHeader(a:show_page_num, a:state) : 1
endfunction
